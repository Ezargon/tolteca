...
class FileRender
{
	...
	public function render(&$model, &$params, $file)
	{
		...
		if ($params->get('make_thumbnail', false) && JFile::exists($thumb_path))
		{
			...
			$this->output .= "<a target='_blank' class=\"download-archive fabrik-filetype-$ext\" title=\"$file\" href=\"$file\">
			<img src=\"$thumb_file\" alt=\"$filename\"></a>";
		}
		else
		{
			$this->output .= "<a target='_blank' class=\"download-archive fabrik-filetype-$ext\" title=\"$file\" href=\"$file\">" . $filename . "</a>";
		}
	}

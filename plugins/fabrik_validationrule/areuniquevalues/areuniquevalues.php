...
class PlgFabrik_ValidationruleAreUniqueValues extends PlgFabrik_Validationrule
{
	...
	public function validate($data, $repeatCounter)
	{
		...
		/*SE COGE LOS ELEMTOS POR PARAMETRO*/
		$onefield = $params->get('areuniquevalues-onefield', '');
		...
		if ((int) $onefield !== 0)
		{
			$oneElementModel = $this->getOtherElement();
			$oneFullName = $oneElementModel->getFullName(true, false);
			$onefield = $oneElementModel->getFullName(false, false);
		}
		else
		{
			// Old fabrik 2.x params stored element name as a string
			$oneFullName = $table->db_table_name . '___' . $onefield;
		}
		...
		if (!empty($onefield))
		{
			// $$$ the array thing needs fixing, for now just grab 0
			$formdata = $elementModel->getForm()->formData;
			$w = JArrayHelper::getValue($formdata, $oneFullName . '_raw', JArrayHelper::getValue($formdata, $oneFullName, ''));
		
			if (is_array($w))
			{
				$w = JArrayHelper::getValue($w, 0, '');
			}
		
			$query->where($onefield . ' = ' . $db->quote($w));
		}		
